...
class InstrumentationTracer {
    ...
    async compile(code, language = 'cpp') {
        ...
        const compileUser = new Promise((resolve, reject) => {
            const args = ['-c', '-g', '-O0', stdFlag, '-fno-omit-frame-pointer',
                '-finstrument-functions', ...includeFlags, sourceFile, '-o', userObj];
        ...
        const compileTracer = new Promise((resolve, reject) => {
            const disableInstrFlag = compiler.includes('clang')
                ? null // clang doesn't recognize -fno-instrument-functions; default is already "off"
                : '-fno-instrument-functions';
            const tracerArgs = ['-c', '-g', '-O0', stdFlag, '-fno-omit-frame-pointer',
                ...includeFlags, this.tracerCpp, '-o', tracerObj];
            if (disableInstrFlag) tracerArgs.splice(5, 0, disableInstrFlag);
    ...
    /**
     * FIXED: Cross-platform step conversion with guaranteed consistent output
     */
    async convertToSteps(events, executable, sourceFile, programOutput, trackedFunctions, inputLinesMap = null) {
        ...
        steps.push({
            ...
            timestamp: events.length > 0 ? (events[0].ts || null) : Date.now(),
        ...
        for (let i = 0; i < events.length; i++) {
            ...
            if (ev.file && ev.line) {
                ...
                // If addr2line cannot resolve (common on Windows), keep the event by
                // mapping it to the user's source file so func_enter/exit aren't dropped.
            ...
            if (!mainStarted && ev.type === 'func_enter') {
                const fnName = this.isMainFunction(info.function) ? 'main' : 'main';
                console.log('✅ Found program entry (treating as main)');
                const mainFrame = this.pushCallFrame(fnName);
            ...
            const isStructural = ev.type === 'func_enter' || ev.type === 'func_exit';
            ...
            if (!isStructural && (!info.file || info.line === 0)) {
                continue;
            }
        ...
        // ==========================================
        // STEP 4: Add output steps BEFORE program_end
        // ==========================================
        const outputSteps = this.createOutputSteps(outputLines, stepIndex);
        steps.push(...outputSteps);
        stepIndex += outputSteps.length;
