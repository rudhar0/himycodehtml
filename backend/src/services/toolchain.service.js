...
class ToolchainService {
  ...
  getIncludeFlags(language = 'cpp') {
    ...
    // 3) Platform include dir (mingw/SDK headers bundled with the toolchain)
    if (absPlatformInclude) flags.push('-isystem', absPlatformInclude);
  ...
  async stageRuntimeDependencies(targetDir) {
    if (!targetDir || this.platform !== 'win32') return [];
    const staged = [];
