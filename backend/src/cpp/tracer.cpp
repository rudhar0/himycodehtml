 ...
 *    Applied to: demangle, json_safe_path, write_json_event, lock_trace, unlock_trace,
...
static NO_INSTRUMENT void lock_trace() {
#if defined(TRACER_USE_STD_MUTEX)
    g_trace_mutex.lock();
#elif defined(TRACER_USE_WIN32_CRITICAL_SECTION)
    init_trace_mutex();
...
static NO_INSTRUMENT void unlock_trace() {
#if defined(TRACER_USE_STD_MUTEX)
...
class NO_INSTRUMENT TraceGuard {
public:
    NO_INSTRUMENT TraceGuard() { lock_trace(); }
    NO_INSTRUMENT ~TraceGuard() { unlock_trace(); }
...
static void NO_INSTRUMENT write_json_event(const char* type, void* addr,
                 const char* func_name, int depth,
                 const char* extra) {
    // Don't write events if already inside tracer
...
void __cyg_profile_func_enter(void* func, void* caller) {
    // Reentrancy guard: prevent infinite recursion from instrumentation
    if (g_inside_tracer) return;
...
void* operator new(std::size_t size) {
    if (g_inside_tracer) return std::malloc(size);
    g_inside_tracer = true;
...
void operator delete(void* ptr) noexcept {
    if (g_inside_tracer) { std::free(ptr); return; }
    g_inside_tracer = true;
